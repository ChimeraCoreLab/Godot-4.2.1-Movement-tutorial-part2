...
@onready var jumpbutton = $"../Control/jump"
@onready var crouchbutton = $"../Control/crouch"
@onready var freelookbutton = $"../Control/freelook"
@onready var flashlightbutton = $"../Control/flashlight"
@onready var sprintbutton = $"../Control/sprint"
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("crouch") or crouchbutton.is_pressed() or $RayCast.is_colliding():
		if is_on_floor():
		...
		$SlidingTimer.stop()
		if Input.is_action_pressed("sprint") or sprintbutton.is_pressed():
	...
	if Input.is_action_pressed("free_look") or !$SlidingTimer.is_stopped() or freelookbutton.is_pressed():
	...
	if Input.is_action_pressed("jump") or jumpbutton.is_pressed() and is_on_floor():
	...
	if flashlightbutton.is_pressed():
		$Neck/Head/Eyes/Camera/SpotLight3D.show()
	else:
		$Neck/Head/Eyes/Camera/SpotLight3D.hide()
